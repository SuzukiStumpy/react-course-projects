        ...
        <div style={containerStyle} className={className}>
            <div style={starContainerStyle}>
                {Array.from({ length: maxRating }, (_, i) => (
                    <Star
                        key={i}
                        onRate={() => !readOnly ? handleRating(i + 1) : null}
                        onHoverIn={() => !readOnly ? setTempRating(i + 1) : null}
                        onHoverOut={() => !readOnly ? setTempRating(0) : null}
...
function Star({ onRate, onHoverIn, onHoverOut, full, color, size }) {
    const starStyle = {
        ...
        height: `${size}px`,
        display: 'block',
        cursor: onHoverIn === null ? 'pointer' : "normal",
    }
